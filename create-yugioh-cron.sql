-- Create Yu-Gi-Oh Card Cron Job
-- This cron job runs every 15 minutes and processes all Yu-Gi-Oh accounts
-- The function respects the 6-hour posting interval per account (via last_posted_at)
-- 
-- Benefits:
-- - Cron runs every 15 minutes (faster retry on failures)
...
-- Remove existing cron job if it exists (ignore error if it doesn't)
DO $$
BEGIN
  PERFORM cron.unschedule('post-yugioh-card');
EXCEPTION
...
SELECT cron.schedule(
  ...
  SELECT net.http_post(
    url := 'https://lxtkpwsxupzkxuhhmvvz.supabase.co/functions/v1/post-yugioh-card?interval_hours=6',
...
-- '0 */6 * * *'   -- Every 6 hours (matches posting interval exactly)
