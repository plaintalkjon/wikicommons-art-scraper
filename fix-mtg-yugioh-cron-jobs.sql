...
-- This script ensures both cron jobs are properly configured
...
-- Note: interval_hours parameter ensures accounts only post every 6 hours
...
-- SELECT jobid, jobname, schedule, active, command::text FROM cron.job WHERE jobname IN ('post-mtg-card', 'post-yugioh-card');
