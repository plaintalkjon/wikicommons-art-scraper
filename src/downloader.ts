...
// Track last download time to enforce delays between downloads
let lastDownloadTime = 0;
const MIN_DELAY_BETWEEN_DOWNLOADS = 1500; // 1.5 seconds between downloads

export async function downloadImage(variant: ImageVariant): Promise<DownloadedImage> {
  // Enforce delay between downloads to respect Wikimedia rate limits
  const now = Date.now();
  const timeSinceLastDownload = now - lastDownloadTime;
  if (timeSinceLastDownload < MIN_DELAY_BETWEEN_DOWNLOADS) {
    const waitTime = MIN_DELAY_BETWEEN_DOWNLOADS - timeSinceLastDownload;
